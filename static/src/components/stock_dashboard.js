...
export class StockDashboard extends Component {
   ...
   async getAuthToken() {
        try {
            const response = await fetch('https://10.10.0.1:3000/proxy/identity');
            if (!response.ok) {
     ...
     async fetchTemperatureData(serialNumber, stateKey) {
        try {
            const response = await fetch(`https://10.10.0.1:3000/proxy/data/${serialNumber}?token=${this.state.token}`);
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
            const data = await response.json();
